...
BOOST_FIXTURE_TEST_CASE(action_receipt_tests, TESTER) { try {
   ...
   set_code( N(test), contracts::lgsio_bios_wasm() );
   set_abi( N(test), contracts::lgsio_bios_abi().data() );
	set_code( N(test), contracts::payloadless_wasm() );
...
#warning re-enable deferred transaction replacement test after bug has been fixed
...

   {
      ...
      // But not anymore. Now it should subjectively fail because testapi@additional permission is not unilaterally satisfied by testapi@lgsio.code.
      dtt_action dtt_act3;
