...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance(N(lgsio.token), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(lgsio.token), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
         :TESTER(),abi_ser(json::from_string(contracts::lgsio_token_abi().data()).as<abi_def>(), abi_serializer_max_time)
      {
         create_account( N(lgsio.token));
         set_code( N(lgsio.token), contracts::lgsio_token_wasm() );

         auto result = push_action(N(lgsio.token), N(create), mutable_variant_object()
                 ("issuer",       lgsio_token)
         ...
         result = push_action(N(lgsio.token), N(issue), mutable_variant_object()
                 ("to",       lgsio_token)
      ...
      static const std::string lgsio_token;
};

const std::string currency_tester::lgsio_token = name(N(lgsio.token)).to_string();
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(lgsio.token), expected.get_symbol(), N(lgsio.token));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(lgsio.token), N(transfer), mutable_variant_object()
         ("from", lgsio_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(lgsio.token), N(transfer), mutable_variant_object()
                                    ("from", lgsio_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE( test_input_quantity, currency_tester ) try {
   ...
   {
      auto trace = transfer(lgsio_token, N(alice), "100.0000 CUR");
