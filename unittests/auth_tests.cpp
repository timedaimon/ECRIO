...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for lgsio reqauth action with alice's spending key
   chain.link_authority("alice", "lgsio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "lgsio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with lgsio reqauth
   chain.unlink_authority("alice", "lgsio", "reqauth");
   ...
   // Link authority for any lgsio action with alice's scud key
   chain.link_authority("alice", "lgsio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "lgsio", "first",  "reqauth");
...
try {
   ...
   // Creating account with lgsio. prefix with privileged account
   chain.create_account("lgsio.test1");

   // Creating account with lgsio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("lgsio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'lgsio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "lgsio", "lgsio.any", "reqauth" );
   chain.link_authority( "bob", "lgsio", "lgsio.any", "reqauth" );
